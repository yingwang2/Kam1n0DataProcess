...
# create block size bins table
blockSizeArr = [f[1] for f in sizeArr]
blockSizeArr.sort()
binBlockSize = max(math.floor(len(blockSizeArr) / limit), 5)
createBins(conn, cur, blockSizeRangeTableName, blockSizeArr, binBlockSize, False, functionsTableName, binsByBlockSize)

# create code size treemap matrix
createTreemap(conn, cur, pairCodeSizeTemp, table, codeSizeRangeTableName, True, codeSizeTreemapName)
# create block size treemap matrix
createTreemap(conn, cur, pairBlockSizeTemp, table, blockSizeRangeTableName, False, blockSizeTreemapName)
# create binary treemap matrix
createBinaryTreemap(conn, cur, binaryIdTable, table, binaryTreemapName)
